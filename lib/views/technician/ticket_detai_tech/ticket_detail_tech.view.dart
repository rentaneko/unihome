...
class TicketDetailTechScreen extends GetWidget<TicketDetailTechController> {
  const TicketDetailTechScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () => goToAndRemoveAll(screen: ROUTE_NAV_BAR_TECH),
      child: Scaffold(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildButtonByStatus(controller.ticket.status.toString()),
              ],
            ),
          ),
        ),
  ...
  Widget _buildButtonByStatus(String status) {
    if (status.toLowerCase().toString().trim() == 'active') {
    ...
    } else if (status.toLowerCase().toString().trim() == 'processing') {
      return ElevatedButton(
        onPressed: () => controller.solveTicket(),
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColor.white,
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(100),
